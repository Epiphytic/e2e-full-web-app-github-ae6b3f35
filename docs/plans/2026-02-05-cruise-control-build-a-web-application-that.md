...
- **Backend**: Rust + Axum 0.8 (verify latest stable at https://crates.io/crates/axum before starting), serving both API endpoints and HTML templates
...
3. **CSRF with dual auth methods** — Supporting both Authorization header and cookie-based authentication increases the CSRF attack surface. A multi-layered CSRF defense strategy is required: (a) **SameSite=Strict cookie attribute** (primary defense) — prevents the browser from sending the auth cookie on any cross-origin request, blocking most CSRF vectors; (b) **HX-Request custom header validation** (defense-in-depth) — all state-mutating requests (POST/PUT/DELETE) authenticated via cookie must include the `HX-Request: true` header (sent automatically by htmx), which triggers a CORS preflight for cross-origin requests that will be blocked by the server's CORS policy; (c) **Secure and HttpOnly cookie flags** — Secure ensures cookies are only sent over HTTPS (preventing interception), HttpOnly prevents JavaScript access (mitigating XSS-based cookie theft). These three layers together ensure that even if one defense is bypassed (e.g., SameSite behavior varies across browsers), the others provide protection. See CRUISE-004 for implementation details.
...
```json
{
  ...
  "spawn_instances": [
    ...
    {
      "id": "SPAWN-004A",
      "name": "Base Layout & Auth Templates",
      "use_spawn_team": false,
      "cli_params": "claude --model sonnet --allowedTools Read,Write,Edit,Bash,Glob,Grep --timeout 300",
      "permissions": ["Read", "Write", "Edit", "Bash", "Glob", "Grep"],
    ...
    {
      "id": "SPAWN-004B",
      "name": "Database Editor Templates & View Handlers",
      "use_spawn_team": false,
      "cli_params": "claude --model sonnet --allowedTools Read,Write,Edit,Bash,Glob,Grep --timeout 300",
      "permissions": ["Read", "Write", "Edit", "Bash", "Glob", "Grep"],
      "task_ids": ["CRUISE-008B", "CRUISE-008C", "CRUISE-009"]
    },
    {
      "id": "SPAWN-005",
  ...
  "tasks": [
    ...
    },
    {
      ...
      "description": "Create src/auth.rs with: (1) A function to load the RSA public key from PEM file. (2) An Axum middleware/extractor that extracts the JWT from the Authorization header (Bearer token) or from a cookie, validates it using RS256, and extracts claims (sub, exp, iat). (3) A Claims struct with serde Deserialize. (4) An AuthUser extractor that can be used in handler signatures. (5) Proper error responses (401 Unauthorized) for missing/invalid/expired tokens. (6) A helper function or constant for building the authentication cookie with mandatory security attributes: HttpOnly (prevents XSS-based cookie theft via JavaScript), Secure (ensures the cookie is only sent over HTTPS), and SameSite=Strict (mitigates CSRF attacks by preventing the browser from sending the cookie on cross-origin requests). This helper must be used by any handler that sets or clears the auth cookie (see CRUISE-009). (7) CSRF protection for cookie-based auth: since supporting both Authorization header and cookies increases the CSRF attack surface (see Risk Area 3), a multi-layered defense is mandatory. Layer 1 (primary): The SameSite=Strict attribute on the auth cookie prevents the browser from sending it on cross-origin requests. Layer 2 (defense-in-depth): All state-mutating requests (POST, PUT, DELETE) authenticated via cookie must include the `HX-Request: true` custom header (sent automatically by htmx). The middleware must reject cookie-authenticated state-mutating requests that lack this header with a 403 Forbidden response. This custom header triggers a CORS preflight on cross-origin requests, which the server's CORS policy will block. Layer 3: HttpOnly and Secure cookie flags prevent XSS-based cookie theft and HTTP interception respectively. The middleware must enforce all three layers and log rejected requests for security monitoring. Include unit tests for token validation with valid and expired tokens, and for CSRF protection (verify that cookie-auth requests without HX-Request header on mutating endpoints are rejected, and verify that the SameSite=Strict attribute is present on all Set-Cookie headers for the auth cookie).",
      ...
      "acceptance_criteria": [
        ...
        "Unit tests verify that the auth cookie Set-Cookie header includes SameSite=Strict, HttpOnly, and Secure attributes",
        "Unit tests verify that cookie-authenticated mutating requests without HX-Request header are rejected with 403",
    ...
    },
    {
      ...
      "permissions": ["Read", "Write", "Edit", "Bash", "Glob", "Grep"],
      "cli_params": "claude --model sonnet --allowedTools Read,Write,Edit,Bash,Glob,Grep",
      "spawn_instance": "SPAWN-004A"
    },
    {
      "id": "CRUISE-008B",
      "subject": "Create table detail template and table-level htmx partials",
      "description": "Create the MiniJinja templates for table-level views in the database editor UI: (1) table_detail.html — shows table schema and rows with edit/delete controls, extending base.html. (2) partials/table_list.html — htmx partial for table list updates. (3) partials/table_schema.html — htmx partial for schema display. Use htmx attributes (hx-get, hx-post, hx-delete, hx-target, hx-swap) for dynamic updates without full page reloads.",
      "blocked_by": ["CRUISE-008A", "CRUISE-007A"],
      "complexity": "medium",
      "acceptance_criteria": [
        "table_detail.html shows schema and rows with edit/delete controls",
        "partials/table_list.html exists for table list updates",
        "partials/table_schema.html exists for schema display",
    ...
    {
      "id": "CRUISE-008C",
      "subject": "Create row-level htmx partials and form templates",
      "description": "Create the MiniJinja templates for row-level views and data entry forms: (1) partials/row_list.html — htmx partial for row listing with pagination. (2) partials/add_column_form.html — htmx partial for adding a column (name, type, nullable, default). (3) partials/add_row_form.html — htmx partial for adding a row. Use htmx attributes (hx-get, hx-post, hx-delete, hx-target, hx-swap) for dynamic updates without full page reloads.",
      "blocked_by": ["CRUISE-008B", "CRUISE-007B"],
      "complexity": "medium",
      "acceptance_criteria": [
        "partials/row_list.html exists for row listing with pagination",
        "partials/add_column_form.html exists for adding a column",
        "partials/add_row_form.html exists for adding a row",
        "Forms use hx-post/hx-delete for AJAX submissions",
        "Row list and forms update without full page reloads",
        "Partials are compatible with the table detail template from CRUISE-008B"
      ],
      "permissions": ["Read", "Write", "Edit", "Bash", "Glob", "Grep"],
      "cli_params": "claude --model sonnet --allowedTools Read,Write,Edit,Bash,Glob,Grep",
      "spawn_instance": "SPAWN-004B"
    },
    {
      ...
      "blocked_by": ["CRUISE-007A", "CRUISE-007B", "CRUISE-008C"],
      "complexity": "medium",
      "acceptance_criteria": [
...
CRUISE-001 (.gitignore)
  ...
  │           └── CRUISE-008B (Table detail & table partials) ←── CRUISE-007A
  │                 └── CRUISE-008C (Row partials & forms) ←── CRUISE-007B
...
| SPAWN-004A | 008A | Base layout & auth templates — foundation for all other templates | No — templates are low-risk |
| SPAWN-004B | 008B, 008C, 009 | Database editor templates & view handlers — tightly coupled data display/editing UI | No — templates are low-risk |
